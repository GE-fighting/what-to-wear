...
// Create 创建穿搭单品
func (r *outfitItemRepository) Create(item *models.OutfitItem) error {
	return r.db.Create(item).Error
}

// GetByID 根据ID获取穿搭单品
func (r *outfitItemRepository) GetByID(id uint) (*models.OutfitItem, error) {
...
// Update 更新穿搭单品
func (r *outfitItemRepository) Update(item *models.OutfitItem) error {
	return r.db.Save(item).Error
}

// Delete 删除穿搭单品
func (r *outfitItemRepository) Delete(id uint) error {
	return r.db.Delete(&models.OutfitItem{}, id).Error
}

// GetByOutfitID 根据穿搭ID获取所有单品
func (r *outfitItemRepository) GetByOutfitID(outfitID uint) ([]models.OutfitItem, error) {
	var items []models.OutfitItem
	err := r.db.Where("outfit_id = ?", outfitID).
...
func (r *outfitItemRepository) GetByClothingItemID(clothingItemID uint) ([]models.OutfitItem, error) {
	var items []models.OutfitItem
...
func (r *outfitItemRepository) GetByRole(outfitID uint, role string) ([]models.OutfitItem, error) {
	var items []models.OutfitItem
	err := r.db.Where("outfit_id = ? AND item_role = ?", outfitID, role).
...
func (r *outfitItemRepository) CreateBatch(items []models.OutfitItem) error {
	if len(items) == 0 {
		return nil
	}
...
func (r *outfitItemRepository) DeleteByOutfitID(outfitID uint) error {
	return r.db.Where("outfit_id = ?", outfitID).Delete(&models.OutfitItem{}).Error
}

// UpdateLayerOrders 更新穿搭单品的层次顺序
func (r *outfitItemRepository) UpdateLayerOrders(outfitID uint, items []models.OutfitItem) error {
...
func (r *outfitItemRepository) GetItemUsageCount(clothingItemID uint) (int64, error) {
	var count int64
	err := r.db.Model(&models.OutfitItem{}).
...
func (r *outfitItemRepository) GetPopularItems(userID uint, limit int) ([]models.ClothingItem, error) {
	var items []models.ClothingItem

	query := r.db.Model(&models.ClothingItem{}).
...
func (r *outfitItemRepository) GetOutfitItemsWithDetails(outfitID uint) ([]models.OutfitItem, error) {
	var items []models.OutfitItem
	err := r.db.Where("outfit_id = ?", outfitID).
...
func (r *outfitItemRepository) GetItemsByRole(userID uint, role string, limit int) ([]models.ClothingItem, error) {
	var items []models.ClothingItem
...
func (r *outfitItemRepository) GetRoleStats(userID uint) (map[string]int64, error) {
	var results []struct {
...
func (r *outfitItemRepository) GetLayerStats(userID uint) (map[int]int64, error) {
	var results []struct {
...
func (r *outfitItemRepository) CheckItemInOutfit(outfitID, clothingItemID uint) (bool, error) {
	var count int64
	err := r.db.Model(&models.OutfitItem{}).
...
func (r *outfitItemRepository) GetMaxLayerOrder(outfitID uint) (int, error) {
	var maxOrder int
	err := r.db.Model(&models.OutfitItem{}).
